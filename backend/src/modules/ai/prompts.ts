export function generateModulePrompt(category: string, existingTitles: string[]): string {
  return `You are the world's greatest storyteller — you carry the souls of every master who came before you.
...
export function generateQuestionsPrompt(content?: string, title?: string): string {
  return `You are an educational assessment creator. Based on this content, create 3-5 quiz questions.
